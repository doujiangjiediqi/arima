...
//      MACoe[0] = tmp[tmp.length - 1];
    ...
    public double[] computeMACoe(double[] originalData, int q) {
        ...
        for (int i = 1; i < len; ++i) {
            ...
            System.arraycopy(result[i], 1, ARCoe, 0, i);
            ARCoe[i] = result[0][i];
        ...
        double[] alpha = new double[p + 1];
        alpha[0] = -1;
        System.arraycopy(bestResult[p], 1, alpha, 1, p);
        double[] paraGarma = new double[q + 1];
        for (int k = 0; k <= q; ++k) {
            double sum = 0.0;
            for (int j = 0; j <= p - k; ++j) {
        ...
        double[] MACoe = new double[q + 1];
        if (MACoe.length - 1 >= 0) System.arraycopy(tmp[q], 1, MACoe, 1, MACoe.length - 1);
        MACoe[0] = tmp[0][q];
        return MACoe;
    }
    ...
    public double[] computeARCoe(double[] originalData, int p) {
        double[] garma = this.autoCovData(originalData, p);
...
/*        Matrix garmaMatrix=DenseMatrix.Factory.zeros(order,order);
        for(int i=0;i<order;i++){
            for(int j=0;j<order;j++){
                garmaMatrix.setAsDouble(garmaArray[i][j],i,j);
            }
        }
        Matrix garmaMatrixInverse=garmaMatrix.pinv();
        Matrix autoReg = garmaMatrixInverse.times()
